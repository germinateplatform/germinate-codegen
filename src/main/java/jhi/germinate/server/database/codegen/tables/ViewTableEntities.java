...
public class ViewTableEntities extends TableImpl<ViewTableEntitiesRecord> {
    ...
    private ViewTableEntities(Name alias, Table<ViewTableEntitiesRecord> aliased, Field<?>[] parameters) {
        super(alias, null, aliased, parameters, DSL.comment("VIEW"), TableOptions.view("create view `view_table_entities` as select `parent`.`id` AS `entity_parent_id`,`parent`.`general_identifier` AS `entity_parent_gid`,`parent`.`name` AS `entity_parent_name`,`parententity`.`name` AS `entity_parent_type`,`child`.`id` AS `entity_child_id`,`child`.`general_identifier` AS `entity_child_gid`,`child`.`name` AS `entity_child_name`,`childentity`.`name` AS `entity_child_type` from (((`germinate_template_4_22_08_24`.`germinatebase` `parent` left join `germinate_template_4_22_08_24`.`germinatebase` `child` on((`parent`.`id` = `child`.`entityparent_id`))) left join `germinate_template_4_22_08_24`.`entitytypes` `parententity` on((`parententity`.`id` = `parent`.`entitytype_id`))) left join `germinate_template_4_22_08_24`.`entitytypes` `childentity` on((`childentity`.`id` = `child`.`entitytype_id`))) where ((`parent`.`id` is not null) and (`child`.`id` is not null))"));
    }
