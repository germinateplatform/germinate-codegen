...
public class ViewStatsTaxonomy extends TableImpl<ViewStatsTaxonomyRecord> {
    ...
    private ViewStatsTaxonomy(Name alias, Table<ViewStatsTaxonomyRecord> aliased, Field<?>[] parameters) {
        super(alias, null, aliased, parameters, DSL.comment("VIEW"), TableOptions.view("create view `view_stats_taxonomy` as select `t`.`genus` AS `genus`,`t`.`species` AS `species`,`t`.`subtaxa` AS `subtaxa`,`t`.`count` AS `count` from (select `germinate_template_4_22_08_10`.`taxonomies`.`genus` AS `genus`,`germinate_template_4_22_08_10`.`taxonomies`.`species` AS `species`,`germinate_template_4_22_08_10`.`taxonomies`.`subtaxa` AS `subtaxa`,count(1) AS `count` from (`germinate_template_4_22_08_10`.`germinatebase` left join `germinate_template_4_22_08_10`.`taxonomies` on((`germinate_template_4_22_08_10`.`taxonomies`.`id` = `germinate_template_4_22_08_10`.`germinatebase`.`taxonomy_id`))) where ((`germinate_template_4_22_08_10`.`taxonomies`.`genus` is not null) and (`germinate_template_4_22_08_10`.`germinatebase`.`entitytype_id` = 1)) group by `germinate_template_4_22_08_10`.`taxonomies`.`id` union select 'N/A' AS `genus`,NULL AS `species`,NULL AS `subtaxa`,count(1) AS `count` from (`germinate_template_4_22_08_10`.`germinatebase` left join `germinate_template_4_22_08_10`.`taxonomies` on((`germinate_template_4_22_08_10`.`taxonomies`.`id` = `germinate_template_4_22_08_10`.`germinatebase`.`taxonomy_id`))) where (isnull(`germinate_template_4_22_08_10`.`taxonomies`.`genus`) and (`germinate_template_4_22_08_10`.`germinatebase`.`entitytype_id` = 1)) group by `germinate_template_4_22_08_10`.`taxonomies`.`id`) `t` order by `t`.`count` desc"));
    }
