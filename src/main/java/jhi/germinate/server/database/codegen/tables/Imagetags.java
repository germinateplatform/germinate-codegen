/*
 * This file is generated by jOOQ.
 */
package jhi.germinate.server.database.codegen.tables;


import java.sql.Timestamp;
import java.util.Arrays;
import java.util.List;

import javax.annotation.Generated;

import jhi.germinate.server.database.codegen.GerminateDb;
import jhi.germinate.server.database.codegen.tables.records.ImagetagsRecord;

import org.jooq.Field;
import org.jooq.Identity;
import org.jooq.Name;
import org.jooq.Schema;
import org.jooq.Table;
import org.jooq.TableField;
import org.jooq.UniqueKey;
import org.jooq.impl.DSL;
import org.jooq.impl.Internal;
import org.jooq.impl.TableImpl;


// @formatter:off
/**
 * This class is generated by jOOQ.
 */
@Generated(
    value = {
        "http://www.jooq.org",
        "jOOQ version:3.11.9"
    },
    comments = "This class is generated by jOOQ"
)
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class Imagetags extends TableImpl<ImagetagsRecord> {

    private static final long serialVersionUID = 1101545343;

    /**
     * The reference instance of <code>germinate_db.imagetags</code>
     */
    public static final Imagetags IMAGETAGS = new Imagetags();

    /**
     * The class holding records for this type
     */
    @Override
    public Class<ImagetagsRecord> getRecordType() {
        return ImagetagsRecord.class;
    }

    /**
     * The column <code>germinate_db.imagetags.id</code>.
     */
    public final TableField<ImagetagsRecord, Integer> ID = createField("id", org.jooq.impl.SQLDataType.INTEGER.nullable(false).identity(true), this, "");

    /**
     * The column <code>germinate_db.imagetags.tag_name</code>.
     */
    public final TableField<ImagetagsRecord, String> TAG_NAME = createField("tag_name", org.jooq.impl.SQLDataType.VARCHAR(255).nullable(false), this, "");

    /**
     * The column <code>germinate_db.imagetags.created_on</code>.
     */
    public final TableField<ImagetagsRecord, Timestamp> CREATED_ON = createField("created_on", org.jooq.impl.SQLDataType.TIMESTAMP.defaultValue(org.jooq.impl.DSL.field("CURRENT_TIMESTAMP", org.jooq.impl.SQLDataType.TIMESTAMP)), this, "");

    /**
     * The column <code>germinate_db.imagetags.updated_on</code>.
     */
    public final TableField<ImagetagsRecord, Timestamp> UPDATED_ON = createField("updated_on", org.jooq.impl.SQLDataType.TIMESTAMP.defaultValue(org.jooq.impl.DSL.field("CURRENT_TIMESTAMP", org.jooq.impl.SQLDataType.TIMESTAMP)), this, "");

    /**
     * Create a <code>germinate_db.imagetags</code> table reference
     */
    public Imagetags() {
        this(DSL.name("imagetags"), null);
    }

    /**
     * Create an aliased <code>germinate_db.imagetags</code> table reference
     */
    public Imagetags(String alias) {
        this(DSL.name(alias), IMAGETAGS);
    }

    /**
     * Create an aliased <code>germinate_db.imagetags</code> table reference
     */
    public Imagetags(Name alias) {
        this(alias, IMAGETAGS);
    }

    private Imagetags(Name alias, Table<ImagetagsRecord> aliased) {
        this(alias, aliased, null);
    }

    private Imagetags(Name alias, Table<ImagetagsRecord> aliased, Field<?>[] parameters) {
        super(alias, null, aliased, parameters, DSL.comment(""));
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Schema getSchema() {
        return GerminateDb.GERMINATE_DB;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Identity<ImagetagsRecord, Integer> getIdentity() {
        return Internal.createIdentity(jhi.germinate.server.database.codegen.tables.Imagetags.IMAGETAGS, jhi.germinate.server.database.codegen.tables.Imagetags.IMAGETAGS.ID);
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UniqueKey<ImagetagsRecord> getPrimaryKey() {
        return Internal.createUniqueKey(jhi.germinate.server.database.codegen.tables.Imagetags.IMAGETAGS, "KEY_imagetags_PRIMARY", jhi.germinate.server.database.codegen.tables.Imagetags.IMAGETAGS.ID);
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public List<UniqueKey<ImagetagsRecord>> getKeys() {
        return Arrays.<UniqueKey<ImagetagsRecord>>asList(
              Internal.createUniqueKey(jhi.germinate.server.database.codegen.tables.Imagetags.IMAGETAGS, "KEY_imagetags_PRIMARY", jhi.germinate.server.database.codegen.tables.Imagetags.IMAGETAGS.ID)
        );
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Imagetags as(String alias) {
        return new Imagetags(DSL.name(alias), this);
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Imagetags as(Name alias) {
        return new Imagetags(alias, this);
    }

    /**
     * Rename this table
     */
    @Override
    public Imagetags rename(String name) {
        return new Imagetags(DSL.name(name), null);
    }

    /**
     * Rename this table
     */
    @Override
    public Imagetags rename(Name name) {
        return new Imagetags(name, null);
    }
// @formatter:on
}
