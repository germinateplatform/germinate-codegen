...
public class ViewStatsCountry extends TableImpl<ViewStatsCountryRecord> {
    ...
    private ViewStatsCountry(Name alias, Table<ViewStatsCountryRecord> aliased, Field<?>[] parameters) {
        super(alias, null, aliased, parameters, DSL.comment("VIEW"), TableOptions.view("create view `view_stats_country` as select ifnull(`germinate_template_4_22_08_23`.`countries`.`country_name`,'UNKNOWN COUNTRY ORIGIN') AS `country`,ifnull(`germinate_template_4_22_08_23`.`countries`.`country_code3`,'UNK') AS `code`,count(1) AS `count` from ((`germinate_template_4_22_08_23`.`germinatebase` left join `germinate_template_4_22_08_23`.`locations` on((`germinate_template_4_22_08_23`.`germinatebase`.`location_id` = `germinate_template_4_22_08_23`.`locations`.`id`))) left join `germinate_template_4_22_08_23`.`countries` on((`germinate_template_4_22_08_23`.`countries`.`id` = `germinate_template_4_22_08_23`.`locations`.`country_id`))) where (`germinate_template_4_22_08_23`.`germinatebase`.`entitytype_id` = 1) group by `germinate_template_4_22_08_23`.`countries`.`id` order by count(1) desc"));
    }
