...
public class ViewStatsCountry extends TableImpl<ViewStatsCountryRecord> {
    ...
    private ViewStatsCountry(Name alias, Table<ViewStatsCountryRecord> aliased, Field<?>[] parameters) {
        super(alias, null, aliased, parameters, DSL.comment("VIEW"), TableOptions.view("create view `view_stats_country` as select ifnull(`germinate_template_4_23_01_09`.`countries`.`country_name`,'UNKNOWN COUNTRY ORIGIN') AS `country`,ifnull(`germinate_template_4_23_01_09`.`countries`.`country_code3`,'UNK') AS `code`,count(1) AS `count` from ((`germinate_template_4_23_01_09`.`germinatebase` left join `germinate_template_4_23_01_09`.`locations` on((`germinate_template_4_23_01_09`.`germinatebase`.`location_id` = `germinate_template_4_23_01_09`.`locations`.`id`))) left join `germinate_template_4_23_01_09`.`countries` on((`germinate_template_4_23_01_09`.`countries`.`id` = `germinate_template_4_23_01_09`.`locations`.`country_id`))) where (`germinate_template_4_23_01_09`.`germinatebase`.`entitytype_id` = 1) group by `germinate_template_4_23_01_09`.`countries`.`id` order by count(1) desc"));
    }
